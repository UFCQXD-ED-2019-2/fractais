...
constexpr float P_PRECISION_F = 1e-5f;

constexpr bool fuzzyEquals(float a, float b) {
    return (a - b >= -P_PRECISION_F && a - b <= P_PRECISION_F);
}
...
public:
    ...
    Pen(unsigned int largura, unsigned int altura);
    Pen(const Pen &p);
    ...
    void  setThickness  (unsigned int espessura)   { thickness = espessura;  }
